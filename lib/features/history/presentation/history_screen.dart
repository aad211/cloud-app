...
class _HistoryScreenState extends ConsumerState<HistoryScreen> {
  ...
  List<AnalysisRecord> _filter(List<AnalysisRecord> records) {
    return switch (_period) {
      ...
      _HistoryPeriod.sevenDays =>
        records.where((r) => _isWithinPastDays(r, 7)).toList(),
      _HistoryPeriod.thirtyDays =>
        records.where((r) => _isWithinPastDays(r, 30)).toList(),
    };
  }
                          ...
                          if (filtered.isNotEmpty) ...[
                            ...
                            for (final (label, groupRecords) in groups) ...[
                              ...
                              for (final record in groupRecords) ...[
                                _RecordCard(
                                  record: record,
                                  timestampLabel: _timestampFmt.format(
                                    record.date,
                                  ),
                                ),
                                const SizedBox(height: 16),
...
class _RecordCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return InkWell(
      onTap: () => context.push('/result?recordId=${record.id}'),
